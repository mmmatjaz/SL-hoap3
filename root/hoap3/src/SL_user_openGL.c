...

{
  ...
  case 23: /* the torso */
    ...
    glScaled(torso_width*0.7,torso_width/2.,length*1.6);
  ...
  case 29: /* the belly */
    ...
    glPushMatrix();	
    glScaled(belly_width*1.1,2*belly_width/3.,.2);
    ...
    glScaled(torso_width,torso_width/2.,length*1.2);
  ...
  case 32: /* the shoulders */

    glScaled(shoulder_width,shoulder_width/2.,length*0.9);
    glTranslated(0.0,0.0,2*length);
    glColor4fv(yellow);
    if (solid)
  ...
  case 138: /* the head */
    ...
    glScaled(head_width/1.5,head_width/1.8,length*0.9);
  ...
  case 137: /* the eye */
    ...
    glScaled(width/1.,width/1.,length*1.5);
    glTranslated(0.0,0.0,0.5);
    glColor4fv(black);
...
static void drawCOMs(SL_Jstate *state, SL_endeff *eff, SL_Cstate *basec,
		SL_quat *baseo) {
	int i, j, n;
	GLfloat color_point[4] = { (float) 0.5, (float) 0.2, (float) 0.2,
			(float) 1.0 };
	GLfloat color_point2[4] = { (float) 1.0, (float) 0.0, (float) 1.0,
			(float) 1.0 };
	double radius = 0.04;
	double sum;
...
//		}
		if (solid)
		{
			glutSolidSphere(radius * links[i].m / 2, 20, 20);
		} else {
			glutWireSphere(radius * links[i].m / 3, 20, 20);
		}
